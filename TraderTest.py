...
import os
os.chdir('/Users/devli/Documents/Trading')
...
def program():
    while True:
        ...
        if ans == '1':
            ...
            print('Please enter a valid number corresponding to one of the above options')


def run_test(full_test,rnd):
    ...
    if not result:
        ...
        performance_output(result[0],result[1],time_taken, result[2])
...
def test(rnd):
    ...
    while q <= points_structure['R'+str(rnd)+'_no_q']:
        question, answer = generate_questions(rnd)
        ans = answers()
        if ans == 'q':
            return
        elif ans == '':
            points+=points_structure['R'+str(rnd)+'_pass']
            history.loc[len(history)] = [question,round(answer,2),ans]
            q+=1
            continue
        elif round(ans,2) == round(float(answer),2):
            points+=points_structure['R'+str(rnd)+'_correct']
        else:
            points+=points_structure['R'+str(rnd)+'_incorrect']
        history.loc[len(history)] = [question,round(answer,2),round(float(ans),2)]
        q+=1
    return points, str(int(points/(points_structure['R'+str(rnd)+'_correct']*\
                            points_structure['R'+str(rnd)+'_no_q'])*100))+'%',history


def full_test():
    start = start_stopwatch()

    result_r1 = run_test(True,1)
    if not result_r1:
        return
    result_r2 = run_test(True,2)
    if not result_r2:
        return
    result_r3 = run_test(True,3)
    if not result_r3:
        return
...
def performance_output(points,percentage,time_taken,all_questions):
